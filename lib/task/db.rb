...
namespace :db do
  ...
  ENV['DATABASE'] = 'db_sinatra' if environment == 'development'
  ENV['DATABASE'] = 'db_sinatra' if environment == 'test'
  ENV['DATABASE'] = 'db_sinatra' if environment == 'production'
  ENV["DATABASE_URL_#{environment.upcase}"]
  migrations_directory = 'db/migrations'
  connection_string = "postgres://sodobi:210402@localhost/#{ENV['DATABASE']}"
